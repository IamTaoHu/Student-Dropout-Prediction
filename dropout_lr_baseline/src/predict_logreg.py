...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="LogReg 3-class prediction (batch).")
    p.add_argument(
        "--input",
        type=str,
    ...
    p.add_argument(
        "--model",
        type=str,
        default=str((OUTPUT_DIR / "logreg_model.joblib").resolve()),
        help="Model path (default: outputs/logreg_model.joblib)",
    )
    p.add_argument(
        "--output",
        type=str,
        default=str((OUTPUT_DIR / "predictions.csv").resolve()),
